...
TEST_METRICS = ('fid', 'fid_mean', 'fid_cov', 'generator', 'discriminator')
...
def cal_train_time(log_dicts, args):
    ...
    for i, log_dict in enumerate(log_dicts):
        ...
        for epoch in log_dict.keys():
            time = np.array(log_dict[epoch]['time'])
            if len(time) != 17:
                continue
            if args.include_outliers:
                all_times.append(time)
            else:
                all_times.append(time[1:])
...
def plot_phase_train(metric, log_dict, epochs, curve_label, json_log):
    ...
    for epoch in epochs:
        ...
        xs_epoch = np.array(iters) / num_iters_per_epoch + (epoch - 1)
        ys_epoch = np.array(log_dict[epoch][metric][:len(iters)])
        if len(xs_epoch) == len(ys_epoch):
            xs.append(xs_epoch)
            ys.append(ys_epoch)
...
def add_plot_parser(subparsers):
    ...
    parser_plt.add_argument(
        ...
        type=str,
        nargs='+',
        default=['loss', 'loss_disc_fake', 'loss_disc_real', 'loss_disc_fake_g'],
